...
export const NewBoeForm = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault(e)
        console.log((e))
        const user = await boeService.createBoe(street, number, strais, plant, door, typeProperty, propertyPrice, broker, licenseAndFees, reform, holdingCost, interiorDesign, subtotal)
        console.log(user)
    }
        ...
        <>
            <ContainerForm>
                <form onSubmit={handleSubmit}>
                    ...
                    <label>Coste de adquisición:
                        {
                            typeProperty === "oficina" || typeProperty === "local" ?
                                propertyPrice && <p> 7.10% {
                                    (propertyPrice * .071).toFixed(2)
                                }</p> :
                                <p> 3.2% {(propertyPrice * .032).toFixed(2)}</p>
                        }
                    </label>

                    <input type="number" name="costAcquisition" value={typeProperty === "oficina" || typeProperty === "local" ? (propertyPrice * 0.071).toFixed(2) : (propertyPrice * 0.032).toFixed(2)} disable />
                    ...
                    <input type="number" name="subtotal" onChange={(e) => onChange(e)} value={getSubtotalPrice()} disabled />
