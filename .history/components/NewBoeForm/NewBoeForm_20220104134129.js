...
export const NewBoeForm = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault(e)
        try {
            const user = await boeService.create({ street, number, strais, plant, door, typeProperty, propertyPrice, broker, licenseAndFees, reform, holdingCost, interiorDesign, subTotal: getSubtotalPrice(), valoration, costAcquisition })
        } catch (error) {
            console.log(error)
        }
    }
    ...
    const getCostAcquisition = () => {
        ...
        typeProperty === "oficina" || typeProperty === "local" ?
            cost = propertyPrice * 0.071 :
            cost = propertyPrice * 0.032
        setCostAcquisition(cost)
        console.log(cost)
    }
        ...
        <>
            <ContainerForm>
                <form onSubmit={handleSubmit}>
                    ...
                    {/* <input type="number" name="costAcquisition" value={typeProperty === "oficina" || typeProperty === "local" ? (propertyPrice * 0.071).toFixed(2) : (propertyPrice * 0.032).toFixed(2)} onChange={() => onChange(e)} disabled />
                    */}

                    <input type="number" name="costAcquisition" value={typeProperty === "oficina" || typeProperty === "local" ? (propertyPrice * 0.071).toFixed(2) : (propertyPrice * 0.032).toFixed(2)} onChange={(e) => onChange(e)} disabled />
